...
func UserReturnRequest(username string) ([]types.Book, error) {
	...
	rows, err := db.Query(`SELECT b.bookId, b.bookName, b.publisher FROM request r JOIN books b ON r.bookId = b.bookId WHERE r.userId = ? and r.status = 'return requested';`, userId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
